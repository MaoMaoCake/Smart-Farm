...
def upgrade():
    op.create_table(
        ...
        sa.Column('createAt', sa.DateTime, nullable=False, default=sa.func.now()),
        sa.Column('updateAt', sa.DateTime, nullable=False, default=sa.func.now(),
...
def downgrade():
    op.drop_table('user', 'farm', 'farmOwner', 'temperatureSensor', 'AC', 'ACAutomation', 'humiditySensor',
                  'dehumidifier', 'CO2Sensor', 'CO2Controller', 'wateringAutomation', 'light',
                  'farmLightPreset', 'lightCombination', 'lightPresetAutomation', 'MQTTMap', 'waterController')
